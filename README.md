...
```java
return send(event, Map.of("x-idempotency-key", UuidGenerator.generateV7().toString()));
```
...
|---|---|---|---|
| `eventId` | `UUID` | No | Idempotency key (RFC 9562 UUID v7, time-ordered) — auto-generated by the engine if omitted |
...
`com.iris.common.lib.utils.UuidGenerator` generates RFC 9562 UUID version 7 identifiers — time-ordered (embeds a 48-bit millisecond timestamp), unlike the fully random UUID v4 produced by `UUID.randomUUID()`. Use it wherever a new UUID is generated in application code (entity IDs, request IDs, idempotency keys, etc.) instead of `UUID.randomUUID()`, so identifiers sort by creation time and improve database index locality.

Backed by JUG (`com.fasterxml.uuid:java-uuid-generator`), which also guarantees monotonic ordering for UUIDs generated within the same millisecond.
