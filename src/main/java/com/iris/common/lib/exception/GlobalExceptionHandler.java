...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ResponseMapper<?>> globalExceptionHandler(Exception ex, WebRequest request) {
        log.error("Unhandled Service Exception for Request {} RequestId {}", request.getDescription(true), MDC.get(MdcKeys.REQUEST_ID.getKey()), ex);
    ...
    public ResponseEntity<ResponseMapper<?>> illegalArgumentExceptionHandler(Exception ex, WebRequest request) {
        log.error("Invalid argument Exception for Request {} RequestId {}", request.getDescription(true), MDC.get(MdcKeys.REQUEST_ID.getKey()), ex);

        var header = Header.builder().responseCode(HttpStatus.BAD_REQUEST.value())
                .customerMessage("Request failed")
                .responseDesc(ex.getMessage())
                .executionTime(getExecutionTime())
                .build();
